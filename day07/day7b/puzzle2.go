...
// "J" here stands for "Jocker", so the ranking changes a bit
...
func replacedJockerCardCount(cardCount map[string]int) map[string]int {
	if cardCount["J"] > 0 && cardCount["J"] < 5 {
		maxNonJockerCard := ""
		for card := range cardCount {
			if card != "J" {
				if maxNonJockerCard == "" {
					maxNonJockerCard = card
				} else if cardCount[card] > cardCount[maxNonJockerCard] {
					maxNonJockerCard = card
				}
			}
		}
		cardCount[maxNonJockerCard] += cardCount["J"]
		delete(cardCount, "J")
	}
	return cardCount
}
...
func handRank(hand string) int {
	...
	cardCount = replacedJockerCardCount(cardCount)
...
func main() {
	...
	// for i := 0; i < len(hands); i++ {
	// 	fmt.Printf("%s %d rank %d\n",
	// 		hands[i], bets[i], handRank(hands[i]))
	// }
