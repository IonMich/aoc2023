...
def to_next_intersection(maze, start, init_dir=-1, finish=maze_end):
    new_loc = (start[0] + dirs[init_dir][0], start[1] + dirs[init_dir][1])
    nodes = deque([(new_loc, 1)])
    seen = {start}
    while nodes:
        node = nodes.popleft()
        if node[0] in seen:
...
def create_graph(maze, start, dir):
    ...
    while start_nodes:
        ...
        graph[start_node[0]] = graph.get(start_node[0], set()) | {(end, dist)}
        graph[end] = graph.get(end, set()) | {(start_node[0], dist)}
...
print(brute_force_graph_longest_path(maze, maze_start, -1)[1])
